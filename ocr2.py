...
class FocusedInvoiceOCR:
    ...
    def extract_company_name(self, text):
        ...
        # Check first 15 lines for company name
        # for i, line in enumerate(lines[:15]):
        #     line = line.strip()
            # if len(line) < 3 or any(word in line.lower() for word in ['invoice', 'bill', 'date', 'no.', 'page', 'www', 'http']):
            #     continue
    ...
    def extract_date(self, text):
        ...
        date_patterns = [
            ...
            r'(?:Date|Invoice\s*Date|Dated)\s*[:]?\s*(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{4})',
            r'(\d{1,2}[/\-\.]\d{1,2}[/\-\.]\d{2,4})', 
    ...
    def extract_total_amount(self, text):
        ...
        total_patterns = [
            r'(?i)(?:total|grand\s*total|amount\s*due|balance\s*due|net\s*total|final\s*total)\s*[:]?\s*[\$ۣ]?\s*([\d,]+\.?\d{0,2})',
            r'(?i)total\s*amount\s*[:]?\s*[\$ۣ]?\s*([\d,]+\.?\d{0,2})',
            r'(?i)payment\s*amount\s*due\s*[:]?\s*[\$ۣ]?\s*([\d,]+\.?\d{0,2})',
            r'^\s*[\$ۣ]?\s*([\d,]+\.\d{2})\s*$',  # Standalone amounts
            r'(?i)(?:^|\s)total[:]?\s*([\d,]+\.?\d{0,2})\s*$',
            
        ]
        
        # Search entire document for totals
        all_totals = []
        for line in lines:
            line = line.strip()
            if not line or re.search(r'(?i)sub\s*total|subtotal', line):
                continue
        ...
        # Return the largest reasonable total
        if all_totals:
            return str(max(all_totals))
    ...
    def extract_table_data(self, text):
        ...
        # Find table data with relaxed detection
        for i, line in enumerate(lines):
            line_lower = line.lower()
            
            # Start table detection
            if self.is_header_line(line_lower) or any(word in line_lower for word in ['item', 'description', 'qty', 'price']):
                in_table = True
                print(f"\nTable started at line: {line}")
                continue
            
            if in_table:
                # Stop at summary keywords
                if any(word in line_lower for word in ['subtotal', 'tax', 'discount', 'total amount', 'grand total']):
                    print(f"\nTable ended at: {line}")
                    break
                
                # Process lines with numbers
                if re.search(r'\d+', line) and line.strip():
                    print(f"\nProcessing: {line}")
                    parsed_item = self.parse_line_simple(line)
                    if parsed_item:
                        print(f"Extracted: {parsed_item}")
...
def main():
    ...
    try:
        ...
        ocr.display_results(result)
        
        # Optionally save to JSON
        import json
        with open('extracted_invoice_data.json', 'w') as f:
            json.dump(result, f, indent=2)
        print("\nData saved to 'extracted_invoice_data.json'")
